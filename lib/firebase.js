...
const apiFunctions = {
  ...
  swapWidgets: async (widgetA, widgetB) => {
    const currentUser = firebase.auth().currentUser;
    if (!currentUser) {
      return;
    }
    ...
    await firebase
      .database()
      .ref(`/overlays/${currentUser.uid}/widgets/${widgetA.id}/position`)
      .set(positionB);
    await firebase
      .database()
      .ref(`/overlays/${currentUser.uid}/widgets/${widgetB.id}/position`)
      .set(positionA);

    const { widgets } = await apiFunctions.getOverlayData();
    return widgets;
  },
